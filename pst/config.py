CONFIG = {
    "CONSUMER_KEY": 'JPIQgfrt5gTI90PgC2DNoLf44',
    "CONSUMER_SECRET": 'wt1ciclku2cftRrv1WrNY3sidoSbRQ3xSP74fKO1dafT1pVHzn',
    "ACCESS_TOKEN": '15718225-77FWg39DfjuZIMRv4aqfuiEd3tM9TbmBHIFenF2tQ',
    "ACCESS_TOKEN_SECRET": 'qx9uoD5yzsUWeBgzVqIzChO7rruAvNjhomKmqua9nsfpl',
    "SEXISTWORDS": ['whore', 'pussy','cunt', 'skank', 'bitch', 'slut', 'bimbo', 'shrill', 'nagging', 'rape', 'kill'],
    "POLITICIANS": ['HillaryClinton','Hillary Clinton', 'AOC', 'Alexandria Ocasio-Cortez', 'Ocasio-Cortez', 'Elizabeth Warren', 'SenWarren', 'ElizabethWarren', 'NancyPelosi', 'Nancy Pelosi']
}